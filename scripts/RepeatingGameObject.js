/**
 * A class that displays a repeating texture that can optionally be offset
	...
	this.draw = function(dt, canvas, xScroll, yScroll)
	{
		...
		for (var y = 0; y < this.height; y += areaDrawn[1])
		{
			for (var x = 0; x < this.width; x += areaDrawn[0])
			{
				...
				// The first time around you have to start drawing from the middle of the
				// image, subsequent tiles always get drawn from the top or left
